...
def category_management(request):
    if request.method == 'POST':
        ...
        if category_image:
            ...
            if not any(category_image.name.lower().endswith(ext) for ext in valid_extensions):
                messages.error(request, "Please upload a valid image file (.png, .jpeg, .jpg, .webp)!")
                return redirect(category_management) 
