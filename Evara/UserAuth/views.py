...
def forgot_password(request):
    if request.method == 'POST':
        ...
        # Save the OTP in the database (or update if it already exists)
        OTP.objects.update_or_create(
            email=email,
            defaults={'otp': otp, 'expires_at': expires_at}
        )
        ...
        if send_otp_email(email, otp):
            return redirect('verify_forgot_password', email=email)  # Redirect to OTP verification page
        else:
...
def verify_forgot_password(request, email):
    if request.method == 'POST':
        entered_otp = request.POST.get('otp')
